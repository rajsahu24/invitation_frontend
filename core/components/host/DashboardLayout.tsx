  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: 'var(--color-bg-primary)' }}>
      ...
      <main className="flex-1 max-w-[1920px] mx-auto w-full p-4 lg:p-6 grid gap-6 grid-cols-1 lg:grid-cols-12 overflow-hidden">
        ...
        <aside className="lg:col-span-3 flex flex-col gap-3 h-fit max-h-full overflow-y-auto custom-scrollbar">
          {sidebar}
        </aside>

        {/* Center Panel (Editable Details) */}
        <section className="lg:col-span-5 flex flex-col gap-6 h-fit max-h-full overflow-y-auto custom-scrollbar">
          {main}
        </section>

        {/* Right Panel (Preview) */}
        <aside className="lg:col-span-4 h-fit lg:sticky lg:top-5">
          {rightPanel}
        </aside>
